...
public class PalantiriManager {
    ...
    public Palantir acquire() {
    	...
    	/*
    	 * "Similar to what you did in assignment one but you use a concurrent hashmap" - Teach
    	 * methods = put and replace
    	 */
    	...
    	for (Map.Entry<Palantir, Boolean> entry : mPalantiriMap.entrySet()) {
    		...
    		if(entry.getValue() == true) {
    			//.replace(key, false)
    			entry.replace(entry.getKey(), false);
    			return entry.getKey();
    		}
    	}
    ...
    public void release(final Palantir palantir) {
    	...
    	if(palantir != null) {
    		...
    		final boolean values = mPalantiriMap.replace(palantir, false, true);
