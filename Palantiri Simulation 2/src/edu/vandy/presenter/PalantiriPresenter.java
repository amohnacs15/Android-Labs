...
public class PalantiriPresenter 
    ...
    private ThreadFactory mThreadFactory =
        new ThreadFactory() {
            ...
            public Thread newThread(Runnable runnable) {
            	...
            	Thread beingThread = new BeingThread(runnable, beingCount, this);
    ...
    private void createAndExecuteBeingsTasks(int beingCount) {
    	...
    	/*
    	 * When you create a ThreadPoolExecutor you need to set a fixed sized thread pool (being count)n)
    	 * ThreadPool will use the factory to make the appropriate kind of thread
    	 * 
    	 */
    	
    	mBeingTasks = new BeingTasks<BeingAsyncTask>();
    	
    	for(; i < beingCount; i++) {
    		BeingAsyncTask beingAsyncTask = new BeingAsyncTask(i, mExitBarrier);
    		mBeingTasks.add(beingAsyncTask);
    	}
    	
    	//ThreadPoolExecutor(int corePoolSize, int maximumPoolSize, long keepAliveTime, 
    	//TimeUnit unit, BlockingQueue<Runnable> workQueue, ThreadFactory threadFactory)
    	ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(beingCount, beingCount,
    			time, timeunit, new LinkedBlockingQueue(), mThreadFactory);
    	
    	for(; i < mBeingsTasks.size(); i++) {
    		t.executeExecutor(threadPoolExecutor, i, mExitBarrier);
    	}
